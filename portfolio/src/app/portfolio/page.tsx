...
import { X, Info, Loader2, ChevronLeft, ChevronRight } from "lucide-react"; 
...
// Interactive Drag & Button Carousel Component for Mockups
const MockupCarousel = ({ mockups, setExpandedMockup }: { mockups: string[], setExpandedMockup: (m: string) => void }) => {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    // Prevent click event if the user is actually dragging
    if (Math.abs(walk) > 10) {
      setHasDragged(true);
    }
  ...
  const scroll = (direction: 'left' | 'right') => {
    if (!carouselRef.current) return;
    const scrollAmount = carouselRef.current.clientWidth * 0.5; // Scroll by half the container width
      ...
      >
        {mockups.map((mockup, mIdx) => (
          <div 
            ...
            onClick={() => {
              if (!hasDragged) setExpandedMockup(mockup);
            }}
          >
            {/* pointer-events-none prevents native HTML ghost-image dragging from interfering */}
            <Image 
              src={mockup} 
                        ...
                        <div key={idx} className={`space-y-12 sm:space-y-16 ${idx > 0 ? "pt-16 border-t border-gray-200" : ""}`}>
                          ...
                          {concept.mockups && concept.mockups.length > 0 && (
                            <MockupCarousel 
                              ...
                              setExpandedMockup={setExpandedMockup} 
                            />
                          )}
                        </div>
                      ));
